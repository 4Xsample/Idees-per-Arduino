...
void setup() {
    ...
    while (WiFi.status() != WL_CONNECTED) { //Espera a connectar-se a la xarxa
        delay(1000);
  ...
  void loop() {
    ...
    if (httpCode == 200) { //Si la resposta és correcta (200 OK)
        ...
        if (!error) { //Si no hi ha hagut cap error en l'anàlisi
            if (!jsonDoc["stream"].isNull()) { //Si hi ha un stream en directe
